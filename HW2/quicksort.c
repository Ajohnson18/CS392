...
static int lomuto(void *array, int left, int right, size_t elem_sz,
	...
	for (size_t  i = left + 1; i <= right; ++i) {
	   	void *curr = (char*)array + (i * elem_sz);
	    if (comp(p, curr) >= 0) {
	      s++;
	      swap((char*)array + (s * elem_sz), curr, elem_sz);
	    }
	  }
...
static void quicksort_helper(void *array, int left, int right, size_t elem_sz,
                             int (*comp) (const void*, const void*)) {
    if (left < right) {
      int m = lomuto(array, left, right, elem_sz, comp);
      quicksort_helper(array, left, m-1, elem_sz, comp);
      quicksort_helper(array, m+1, right, elem_sz, comp);
    }
}
...
// 	quicksort(a, 6, sizeof(char*), (int (*)(const void*, const void*))(str_cmp));
